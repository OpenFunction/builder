...
echo "> Building base dotnet6common image"
docker build -t "dotnet6common" - < "${DIR}/parent.Dockerfile"
echo "> Building base dotnet6run image"
docker build -t "dotnet6run" - < "${DIR}/parent.cbl-mariner2.0-distroless.Dockerfile"
echo "> Building openfunctiondev/buildpacks-dotnet6-run:$TAG"
docker build --build-arg "from_image=dotnet6common" -t "openfunctiondev/buildpacks-dotnet6-run:$TAG" - < "${DIR}/run.Dockerfile"
echo "> Building openfunctiondev/buildpacks-dotnet6-build:$TAG"
docker build --build-arg "from_image=dotnet6common" -t "openfunctiondev/buildpacks-dotnet6-build:$TAG" -f "${DIR}/build.Dockerfile" "${TEMP}"
