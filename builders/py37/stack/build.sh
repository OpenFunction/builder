...
echo "> Building base py37common image"
docker build -t "py37common" - < "${DIR}/parent.Dockerfile"
echo "> Building openfunctiondev/buildpacks-py37-run:$TAG"
docker build --build-arg "from_image=py37common" -t "openfunctiondev/buildpacks-py37-run:$TAG" - < "${DIR}/run.Dockerfile"
echo "> Building openfunctiondev/buildpacks-py37-build:$TAG"
docker build --build-arg "from_image=py37common" -t "openfunctiondev/buildpacks-py37-build:$TAG" -f "${DIR}/build.Dockerfile" "${TEMP}"
