...
echo "> Building base node10common image"
docker build -t "node10common" - < "${DIR}/parent.Dockerfile"
echo "> Building openfunctiondev/buildpacks-node10-run:$TAG"
docker build --build-arg "from_image=node10common" -t "openfunctiondev/buildpacks-node10-run:$TAG" - < "${DIR}/run.Dockerfile"
echo "> Building openfunctiondev/buildpacks-node10-build:$TAG"
docker build --build-arg "from_image=node10common" -t "openfunctiondev/buildpacks-node10-build:$TAG" -f "${DIR}/build.Dockerfile" "${TEMP}"
