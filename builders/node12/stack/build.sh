...
echo "> Building base node12common image"
docker build -t "node12common" - < "${DIR}/parent.Dockerfile"
echo "> Building openfunctiondev/buildpacks-node12-run:$TAG"
docker build --build-arg "from_image=node12common" -t "openfunctiondev/buildpacks-node12-run:$TAG" - < "${DIR}/run.Dockerfile"
echo "> Building openfunctiondev/buildpacks-node12-build:$TAG"
docker build --build-arg "from_image=node12common" -t "openfunctiondev/buildpacks-node12-build:$TAG" -f "${DIR}/build.Dockerfile" "${TEMP}"
