...
func TestAcceptanceNodeJsFn(t *testing.T) {
	...
	testCases := []acceptance.Test{
		...
		},
		{
			...
			Path:       "/testFunction",
			Env:        []string{"FUNC_NAME=testFunction"},
			MustUse:    []string{nodeYarn, nodeFF},
			MustNotUse: []string{nodeNPM, entrypoint},
		},
		{
...
func TestFailuresNodeJsFn(t *testing.T) {
	...
	testCases := []acceptance.FailureTest{
		...
		{
			Name:      "function fail without valid FUNC_SRC",
			App:       "no_package",
			Env:       []string{"FUNC_NAME=testFunction", "FUNC_SRC=sub_dir"},
			MustMatch: "FUNC_SRC is not currently supported for Node.js buildpacks",
		},
	}
