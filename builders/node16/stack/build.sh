...
echo "> Building base node16common image"
docker build -t "node16common" - < "${DIR}/parent.Dockerfile"
echo "> Building base node16run image"
docker build -t "node16run" - < "${DIR}/parent.alpine.Dockerfile"
echo "> Building openfunctiondev/buildpacks-node16-run:$TAG"
docker build --build-arg "from_image=node16run" -t "openfunctiondev/buildpacks-node16-run:$TAG" - < "${DIR}/run.Dockerfile"
echo "> Building openfunctiondev/buildpacks-node16-build:$TAG"
docker build --build-arg "from_image=node16common" -t "openfunctiondev/buildpacks-node16-build:$TAG" -f "${DIR}/build.Dockerfile" "${TEMP}"