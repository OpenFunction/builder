...
echo "> Building base node14common image"
docker build -t "node14common" - < "${DIR}/parent.Dockerfile"
echo "> Building openfunctiondev/buildpacks-node14-run:$TAG"
docker build --build-arg "from_image=node14common" -t "openfunctiondev/buildpacks-node14-run:$TAG" - < "${DIR}/run.Dockerfile"
echo "> Building openfunctiondev/buildpacks-node14-build:$TAG"
docker build --build-arg "from_image=node14common" -t "openfunctiondev/buildpacks-node14-build:$TAG" -f "${DIR}/build.Dockerfile" "${TEMP}"
